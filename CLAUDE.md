...
**坑 19：Spark 的射线拾取在第一帧渲染之前静默返回空（2026-09-19，测量功能）**
- 症状：切资产后立刻点，`raycaster.intersectObject(mesh)` 恒为空，每次只花约 1 ms（正常要 100~300 ms）；
  不报错。
- 原因：`SplatMesh.raycast` 按 `mesh.context.numSplats.value` 遍历，而这个值要等 SparkRenderer
  渲染过一帧才填上。后台标签页 rAF 被暂停（坑 13），所以自动化测试里特别容易撞上。
- 修复：点击时先查 `context.numSplats.value`，为 0 就提示「场景还没渲染出来」，不去报「没点中」。
- 诊断特征：**耗时 ~1 ms 的 miss = 根本没遍历**；耗时正常的 miss 才是真没打中。

**坑 20：拾取命中率可能与阈值完全无关（2026-09-19，测量功能）**
- 症状：riverview 上 12 个网格点在 `minRaycastOpacity` 为 0.2 / 0.1 / 0.05 / 0.02 时都只命中 1 个，
  而 test.sog 从 5 → 9 → 12 随阈值上升。
- 原因（未完全确认）：两种可能都会让命中率与阈值无关 —— 当时的取景是从场景外斜看一块 7.1×0.95×4.04 的
  薄板，大部分像素本来就是背景；另外 riverview 的高斯极小（坑 12，scale 中位 0.0035），
  无限细的射线容易从高斯之间穿过去，渲染时看着连续是因为混合，3D 里是稀疏的。
